...
contract Router {
        ...
        )
    {
        ...
        address pairAddress = TeddyswapLibrary.pairFor(
            address(factory),
            tokenA,
            tokenB
        );
    ...
    function removeLiquidity(
        ...
        address pair = TeddyswapLibrary.pairFor(
            address(factory),
            tokenA,
            tokenB
        );
    ...
    function swapExactTokensForTokens(
        ...
        amounts = TeddyswapLibrary.getAmountsOut(
            address(factory),
            amountIn,
            path
        );
        ...
        _safeTransferFrom(
            path[0],
            msg.sender,
            TeddyswapLibrary.pairFor(address(factory), path[0], path[1]),
            amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        amounts = TeddyswapLibrary.getAmountsIn(
            address(factory),
            amountOut,
            path
        );
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (address token0, ) = TeddyswapLibrary.sortTokens(input, output);
            uint256 amountOut = amounts[i + 1];
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2
                ? TeddyswapLibrary.pairFor(
                    address(factory),
                    output,
                    path[i + 2]
                )
                : to_;
            IPair(
                TeddyswapLibrary.pairFor(address(factory), input, output)
